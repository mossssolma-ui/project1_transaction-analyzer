...
@pytest.fixture
def mock_currency_and_stock() -> None:
    """Фикстура для мока валют и акций"""
    with patch("src.views.get_currency") as mock_currency, \
...
def test_views_info_success(
    ...
    mock_currency_and_stock: Mock
) -> None:
    """Тест успешного формирования JSON-ответа"""
    caplog.set_level(logging.INFO)

    with patch("src.views.get_data_period_from_file") as mock_get_data, \
         patch("src.views.get_convert_dates") as mock_convert:
...
def test_views_info_empty_dataframe(
    caplog: LogCaptureFixture,
    mock_currency_and_stock: Mock
) -> None:
...
def test_views_info_exception_in_subfunction(
    caplog: LogCaptureFixture
) -> None:
    ...
    with patch("src.views.get_data_period_from_file") as mock_get_data, \
         patch("src.views.get_convert_dates") as mock_convert, \
         patch("src.views.get_card_spending_summary", side_effect=Exception("Test error")):
...
def test_views_info_file_not_found_excel(
    caplog: LogCaptureFixture,
    mock_currency_and_stock: Mock
) -> None:
...
def test_views_info_invalid_date_format(
    caplog: LogCaptureFixture,
    mock_currency_and_stock: Mock
) -> None:
